...
def detect_entity_boundaries(text):
    ...
    Strategy: 
    1. First split on commas (common delimiter in badly formatted lists)
    2. Then apply pattern matching to each segment
    """
    
    # Step 1: Split on commas first
    comma_segments = [s.strip() for s in text.split(',') if s.strip()]
    
    all_entities = []
    
    for segment in comma_segments:
        # Step 2: Apply pattern detection to each segment
        segment_entities = _detect_by_patterns(segment)
        all_entities.extend(segment_entities)
    
    return all_entities if all_entities else [text]

def _detect_by_patterns(text):
    """
    Internal function to detect entities by organizational pattern endings.
    """
    ...
    org_endings = [
        ...
        r'\bChamber\s+[Oo]f\s+Commerce(?:\s+(?:and|&)\s+Industry)?',
        r'\bChurch\s+[Oo]f\s+Christ',
        r'\bChurch\s+[Oo]f\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?',
        r'\bDepartment\s+[Oo]f\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?',
        r'\bMinistry\s+[Oo]f\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?',
        r'\bBoard\s+[Oo]f\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?',
        r'\bBureau\s+[Oo]f\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?',
        r'\bCouncil\s+(?:[Oo]n|[Oo]f)\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*',
        
        # === Committees and Congressional bodies ===
        r'\bInaugural\s+Committee',
        r'\bBuilding\s+Congress',
        r'\b(?:Select|Standing|Joint|Special)\s+Committee(?:\s+(?:on|for)\s+[A-Za-z\s]+)?',
        r'\bCommittee(?:\s+(?:for|on|of)\s+[A-Za-z\s]+)?',
        
        # === Organizations with modifiers ===
        r'\b(?:Trade|Business|Professional)\s+Association',
        r'\bPolicy\s+(?:Institute|Center|Centre)',
        r'\bResearch\s+(?:Institute|Center|Centre)',
        r'\bAction\s+(?:Fund|Network|Coalition)',
        
        # === Single word patterns (LOWER PRIORITY) ===
        r'\bInstitutes?',
        r'\bFoundations?',
        r'\bCoalitions?',
        r'\bAlliances?',
    ...
    for pattern in org_endings:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            ...
            if full_match_end < len(text):
                ...
                # If next word starts with capital OR we're at end, this is a boundary
    ...
    # Sort by position (keeping duplicates for now)
    entity_ends.sort(key=lambda x: x[0])
    
    # Remove overlapping matches - keep longer matches
    filtered_ends = []
    last_end = -1
    for end_pos, matched_text in entity_ends:
        # Only add if this doesn't overlap with previous match
        if end_pos > last_end + 1:  # +1 to allow adjacent matches
            filtered_ends.append((end_pos, matched_text))
            last_end = end_pos
    ...
    for end_pos, matched_ending in filtered_ends:
